...
const getTodos = async () => {
  // puedo pasar parametros
  try {
    const res = await axios.get(baseUrl); // el await te devuelve el resultado no la promesa
    lista = res.data; // guardandando la info que obtuve de arriba
    armarHtml(); // y despues ejecuto directamente la funcion va todo en orden
  } catch (err) {
    // por parametro no se pasa la funcion
    handleError(err);
  }
};
...
const getTodo = async id => {
  try {
    const info = await axios.get(`${baseUrl}/${id}`);
    todo = info.data;
  } catch (err) {
    handleError(err);
  }
...
const armarHtml = () => {
  ...
  lista.map(item => {
    //mientas loopea va ir creando lo ul, li, span, que me loope por cada item de lista
